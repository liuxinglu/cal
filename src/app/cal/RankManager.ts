module app {
	export class RankManager {
		private _rankSplits:Array<Array<string>> = [
		...
		private _gradeSeconds:Array<Array<number>> = [
		...
		private _gradeRate:Array<number> = [0.1, 0.1, 0.1, 0.1, 0.2, 0.2, 0.2, 0.3, 0.3];
		private _titles:Array<Array<string>> = [
		...
		getIndexByKnowledge(knowledge:Knowledge):number {
			for(let i = 0; i < this._rankSplits.length; i++) {
				for(let j = 0; j < this._rankSplits[i].length; j++) {
		...
		getJudgeTitleIndex(index:number):number {
			...
			let ind = 0;
			for(let i = 0; i < this._gradeSeconds[index].length; i++) {
				if(i == 0) {
		...
		getRankNamesByRateAndTime(knowledge:Knowledge):Array<string> {
			let rate = Math.floor(Cal.cuoTiBen.length * 100 / Cal.curQs.length) / 100;
			if(rate == 0) {//全对
				let index = -1;
				for(let i = 0; i < this._gradeSeconds[Cal.curGrade - 1].length; i++) {
					if(i == 0) {
				...
				let ind = this.getIndexByKnowledge(knowledge);
				if(ind == undefined) {
					if(Cal.curGrade == 4) {
						ind = 7;
